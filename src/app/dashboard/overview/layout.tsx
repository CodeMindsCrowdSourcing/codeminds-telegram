...
export default function OverViewLayout() {
  ...
  const [userActivity, setUserActivity] = useState<
    { name: string; users: number; interactions: number }[]
  >([]);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        if (!botsResponse.ok) {
          toast.error(`Failed to fetch bots: ${botsResponse.statusText}`);
          return;
        }
        const botsData = await botsResponse.json();
        if (!Array.isArray(botsData)) {
          toast.error('Invalid bots data format:', botsData);
          return;
        }
        ...
        for (const bot of botsData) {
          if (!bot || !bot._id) {
            toast.error('Invalid bot data:', bot);
            continue;
          }

          try {
            const usersResponse = await fetch(
              `/api/telegram-bot/${bot._id}/users`
            );
            if (!usersResponse.ok) {
              toast.error(
                `Failed to fetch users for bot ${bot._id}: ${usersResponse.statusText}`
              );
              continue;
            }
            ...
            toast.error(
              `Error fetching users for bot ${bot._id}: ${(error as Error).message}`
            );
          }
        }
        ...
        toast.error('Error fetching stats: ' + (error as Error).message);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    const fetchUsers = async () => {
      ...
      try {
        const response = await fetch(
          `/api/telegram-bot/${selectedBotId}/users`
        );
        if (!response.ok) {
        ...
        if (Array.isArray(data)) {
          ...
          for (let i = 6; i >= 0; i--) {
            ...
            const dayUsers = data.filter((user) => {
              const clickDate = user.clickedAt
                ? new Date(user.clickedAt)
                : null;
              return (
                clickDate &&
