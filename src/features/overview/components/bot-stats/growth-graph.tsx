...
export function GrowthGraph({ data }: GrowthGraphProps) {
  ...
  const growthPercent = prevValue ? ((lastValue - prevValue) / prevValue) * 100 : 0;
  ...
  return (
    <Card className='@container/card'>
      ...
      <div className='flex w-full items-start gap-2 text-sm px-6 pb-6'>
        <div className='grid gap-2'>
          <div className='flex items-center gap-2 leading-none font-medium'>
            {growthPercent > 0 ? 'Growing' : 'Declining'} by {Math.abs(growthPercent).toFixed(1)}% this period{' '}
